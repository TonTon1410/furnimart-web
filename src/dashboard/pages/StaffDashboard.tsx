...
const CardSkeleton = () => (
  <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 animate-pulse h-full">
...
const TaskCard: React.FC<TaskCardProps> = ({ title, value, icon, colorClass, bgClass, loading, actionLabel }) => {
  ...
  return (
    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300 flex items-center justify-between group">
      <div>
        ...
        {actionLabel && (
           <span className="text-xs font-semibold text-gray-400 mt-1 inline-block group-hover:text-gray-600 transition-colors cursor-pointer">
             {actionLabel} &rarr;
           </span>
        )}
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 p-4 md:p-6 lg:p-8 font-sans text-slate-900">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Bàn Làm Việc</h1>
          <p className="text-gray-500 mt-1 text-sm">Xin chào, chúc bạn một ca làm việc hiệu quả.</p>
        </div>
        <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-xl shadow-sm border border-gray-100">
            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
            <span className="text-sm font-semibold text-gray-700">Cửa hàng đang mở cửa</span>
        </div>
      </div>

      {/* 1. Task Overview Cards - Tập trung vào việc cần làm [cite: 65, 67] */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-2 bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="h-[300px] w-full">
                {loading ? <div className="h-full bg-gray-100 rounded animate-pulse"></div> : (
        ...
        <div className="bg-white p-0 rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[380px]">
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-amber-50/50">
                <h2 className="text-lg font-bold text-gray-800">Cần Duyệt Gấp</h2>
                <span className="bg-amber-100 text-amber-700 text-xs font-bold px-2 py-1 rounded-md">{stats?.pendingOrders} đơn</span>
            </div>
            <div className="flex-1 overflow-auto">
                <ul className="divide-y divide-gray-50">
                    {!loading && orders.map(order => (
                        <li key={order.id} className="p-4 hover:bg-gray-50 transition-colors cursor-pointer group">
                            <div className="flex justify-between items-start mb-1">
                                <span className="text-xs font-bold text-gray-400">{order.id}</span>
                                <span className="text-[10px] font-medium text-gray-400">{order.time}</span>
                            </div>
                            <div className="flex justify-between items-center mb-2">
                                <h4 className="text-sm font-semibold text-gray-800">{order.customerName}</h4>
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <button className="opacity-0 group-hover:opacity-100 transition-opacity bg-indigo-600 text-white text-xs px-3 py-1.5 rounded-lg shadow hover:bg-indigo-700 font-medium">
                                    Xử lý
                                </button>
                            </div>
                        </li>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-5 border-b border-gray-100 flex justify-between items-center">
              ...
              <button className="text-gray-400 hover:text-gray-600"><MoreHoriz /></button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-gray-100">
              {!loading && messages.map((msg) => (
                  <div key={msg.id} className={`p-4 hover:bg-gray-50 transition-colors cursor-pointer flex gap-4 ${msg.isUnread ? 'bg-indigo-50/30' : ''}`}>
                      <img src={msg.avatar} alt="" className="w-10 h-10 rounded-full border border-gray-200" />
                      <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start mb-1">
                              <h4 className={`text-sm ${msg.isUnread ? 'font-bold text-gray-900' : 'font-medium text-gray-700'}`}>
                                  {msg.customerName}
                              </h4>
                              <span className="text-[10px] text-gray-400">{msg.time}</span>
                          </div>
                          <p className={`text-xs truncate ${msg.isUnread ? 'text-indigo-600 font-medium' : 'text-gray-500'}`}>
                              {msg.preview}
                          </p>
                      </div>
