        ...
        if (!hideWarehouse) {
          const fetchWarehouses = async () => {
            try {
              ...
              const warehouseList = Array.isArray(res.data)
                ? res.data
                : res.data?.data || [];
      ...
      useEffect(() => {
        if (selectedWarehouseId) {
          const fetchZones = async () => {
            try {
              const res = await zoneService.getZoneByWarehouse(
                selectedWarehouseId
              );
      ...
      useEffect(() => {
        if (selectedZoneId) {
          const fetchLocations = async () => {
            try {
              const res = await locationItemService.getLocationByZone(
                selectedZoneId
              );
              const locationList = Array.isArray(res.data)
                ? res.data
                : res.data?.data || [];
