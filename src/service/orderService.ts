...
class OrderService {
  ...
  async getOrderById(id: number): Promise<OrderItem> {
    try {
      const response: AxiosResponse<ApiResponse<ApiOrder>> =
        await axiosClient.get(`/orders/${id}`);
      const orderData = response.data.data || response.data;
      return this.convertToOrderItem(orderData);
    } catch (error: any) {
      throw new Error(
        this.handleError(error, "Không thể lấy thông tin đơn hàng")
      );
    }
  }
  ...
  async updateOrderStatus(id: number, status: string): Promise<OrderItem> {
    try {
      ...
      const response: AxiosResponse<ApiResponse<ApiOrder>> =
        await axiosClient.put(`/orders/status/${id}?status=${status}`);
      const orderData = response.data.data || response.data;
      return this.convertToOrderItem(orderData);
    } catch (error: any) {
      throw new Error(
        this.handleError(error, "Không thể cập nhật trạng thái đơn hàng")
      );
    }
  }
  ...
  async searchOrdersByCustomer(
    ...
    try {
      ...
      return {
        orders: (responseData.content || []).map((order: ApiOrder) =>
          this.convertToOrderItem(order)
        ),
  ...
  async acceptOrder(id: number, storeId?: string): Promise<void> {
    try {
      ...
      await axiosClient.post(
        `/orders/${id}/manager-decision?${params.toString()}`
      );
    } catch (error: any) {
  ...
  private convertToOrderItem(apiOrder: ApiOrder): OrderItem {
    const totalQuantity =
      apiOrder.orderDetails?.reduce((sum, item) => sum + item.quantity, 0) || 0;

    const currentStatus =
      apiOrder.processOrders?.[apiOrder.processOrders.length - 1]?.status ||
      "PENDING";
    ...
    return {
      ...
      category: "Nội thất",
      shopName:
        apiOrder.user?.fullName || apiOrder.address?.name || "Khách hàng",
