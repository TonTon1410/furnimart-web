...
export const authService = {
  ...
  forgotPassword: async (email: string) => {
    try {
      const res = await axiosClient.post<ForgotPasswordResponse>("/auth/forgot-password", { email })
      if (res.data.status === 200) {
        return res
      }
      throw new Error(res.data.message || "Forgot password response không đúng format")
    } catch (error: any) {
      if (error.response) {
        const message =
          error.response.data?.message || error.response.data?.error || `Server error: ${error.response.status}`
  ...
  resetPassword: async (token: string, newPassword: string) => {
    try {
      const res = await axiosClient.post<ResetPasswordResponse>("/auth/reset-password", {
        token,
        newPassword,
      })
      if (res.data.status === 200) {
      ...
      throw new Error(res.data.message || "Reset password response không đúng format")
    } catch (error: any) {
      if (error.response) {
        const message =
          error.response.data?.message || error.response.data?.error || `Server error: ${error.response.status}`
